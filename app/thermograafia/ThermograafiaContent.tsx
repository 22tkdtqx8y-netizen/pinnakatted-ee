...
export function ThermograafiaContent() {
  const locale = useLocale();
  const t = getMessages(locale).thermografia;
  const faqTitle = getMessages(locale).home.faqTitle;
  const purLabel = getMessages(locale).services.pur.title;
  ...
  return (
    <>
      <Section className="bg-slate-50">
        <Container>
          <div className="mx-auto max-w-3xl text-center">
            <h1 className="text-3xl font-bold tracking-tight text-slate-900 sm:text-4xl">{t.hero.title}</h1>
            ...
            <div className="mt-8 flex flex-wrap items-center justify-center gap-4">
              <Button asChild variant="primary" size="lg" className="min-h-[48px]">
                <Link href="/kontakt?thermograafia=1">{t.cta.primary}</Link>
              </Button>
              <Button asChild size="lg" className="min-h-[48px] border-2 border-slate-300">
                <a href={`tel:${company.phone.replace(/\s/g, "")}`}>{t.helista}</a>
              </Button>
            </div>
            <p className="mt-4 text-sm text-slate-500">{t.cta.microcopy}</p>
          </div>
        </Container>
      </Section>
      ...
      <Section className="bg-white">
        <Container>
          <div className="mx-auto max-w-3xl">
            <h2 className="text-2xl font-bold text-slate-900">{t.what.title}</h2>
            <ul className="mt-4 space-y-2 text-slate-600">
              {t.what.points.map((point) => (
      ...
      <Section className="bg-slate-50">
        <Container>
          <div className="mx-auto max-w-3xl">
            <h2 className="text-2xl font-bold text-slate-900">{t.whenOrdered.title}</h2>
            <ul className="mt-6 space-y-3">
              {t.whenOrdered.items.map((item) => (
      ...
      <Section className="bg-white">
        <Container>
          <div className="mx-auto max-w-3xl">
            <h2 className="text-2xl font-bold text-slate-900">{t.pricing.title}</h2>
            ...
            <p className="mt-3 text-slate-600">{t.pricing.note1}</p>
            <p className="mt-1 text-slate-600">{t.pricing.note2}</p>
            <div className="mt-6">
      ...
      <Section className="bg-slate-50">
        <Container>
          <div className="mx-auto max-w-3xl">
            <h2 className="text-2xl font-bold text-slate-900">{t.whatClientGets.title}</h2>
            <ul className="mt-6 space-y-3">
              {t.whatClientGets.items.map((item) => (
                <li key={item} className="flex items-start gap-3">
                  <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-primary-100 text-primary-600">
                    <svg className="h-4 w-4" fill="currentColor" viewBox="0 0 20 20">
                      <path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" />
        ...
        <Container>
          <div className="mx-auto max-w-3xl rounded-xl border border-amber-200 bg-white px-6 py-5">
            <p className="text-slate-800">
              <strong className="text-slate-900">{t.importantLabel}:</strong> {t.importantNote}
            </p>
          </div>
        </Container>
      </Section>
      ...
      <Section className="bg-primary-600">
        <Container>
          <div className="mx-auto max-w-2xl text-center text-white">
            ...
            <p className="mt-3 text-primary-100">{t.cta.microcopy}</p>
            <div className="mt-6 flex flex-wrap justify-center gap-4">
              ...
              <Button asChild size="lg" className="min-h-[48px] border-2 border-white bg-transparent text-white hover:bg-white/10">
                <Link href={`/teenused/${services.pur.slug}`}>{purLabel}</Link>
              </Button>
            </div>
          </div>
        </Container>
      ...
      <Section className="bg-slate-50">
        <Container>
          <div className="mx-auto max-w-xl">
            <h2 className="text-xl font-bold text-slate-900">{t.cta.primary}</h2>
            <p className="mt-2 text-slate-600">{t.cta.microcopy}</p>
            <div className="mt-6">
